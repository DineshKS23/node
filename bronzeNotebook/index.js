...
async function createNotebook(content, path, databricksUrl, accessToken, edit, bronzeNotebookName, projectId, userId) {
    try {
        let overwrite;
...
module.exports = async function (context, req) {
    try {
        ...
        const { content, path, bronzeNotebookName, databricksUrl, accessToken, edit, projectId, userId } = req.body;

        if (!databricksUrl || !accessToken || !projectId || !userId || (!bronzeNotebookName && !edit)) {
            context.res = {
                status: 400,
                body: {
                    status_code: 400,
        ...
        if (!edit) {
            ...
            if (bronzeNotebookName) {
                const sqlInsertQuery = `INSERT INTO projectDetails (bronzeNotebookName) VALUES (@bronzeNotebookName)`;
                await pool.request()
                    .input('bronzeNotebookName', sql.VarChar, bronzeNotebookName)
            ...
            if (bronzeNotebookName) {
                const sqlUpdateQuery = `UPDATE projectDetails SET bronzeNotebookName = @bronzeNotebookName 
