...
async function createNotebook(content, path, databricksUrl, accessToken, edit, matadataValue, projectId, userId) {
    try {
        let overwrite;
...
module.exports = async function (context, req) {
    try {
        ...
        const { content, path, matadataValue, databricksUrl, accessToken, edit, projectId, userId } = req.body;

        if (!databricksUrl || !accessToken || !projectId || !userId || (!matadataValue && !edit)) {
            context.res = {
                status: 400,
                body: {
                    status_code: 400,
        ...
        if (!edit) {
            ...
            if (matadataValue) {
                const sqlInsertQuery = `INSERT INTO projectDetails (matadataValue) VALUES (@matadataValue)`;
                await pool.request()
                    .input('matadataValue', sql.VarChar, matadataValue)
            ...
            if (matadataValue) {
                const sqlUpdateQuery = `UPDATE projectDetails SET matadataValue = @matadataValue 
        ...
        const response = await createNotebook(content, path, databricksUrl, accessToken, edit, matadataValue, projectId, userId);
